...
export function generateForecast({ model = 'XGBoost', district = 'National', until = 2035 }) {
  const mult = modelMultipliers[model] || 1;
  const districtFactor =
    district === 'National' || district === 'National (all districts)'
      ? 1
      : 0.06 + (district.length % 7) * 0.01;

  const start = district === 'National' || district === 'National (all districts)' ? 8180 : 480;
  const growth = district === 'National' || district === 'National (all districts)' ? 240 : 18;

  const series = baseHistorical.map((row) => ({
    year: row.year,
    historical: Math.round(row.historical * districtFactor * (district === 'National' || district === 'National (all districts)' ? 1 : 0.065)),
    predicted: null,
    upper: null,
    lower: null,
  }));

  // Align 2023 historical for district views
  if (district !== 'National' && district !== 'National (all districts)') {
    series.forEach((row, i) => {
      row.historical = Math.round(420 + i * 15 + district.length);
    });
  }
  ...
  const table = [];
  for (let year = 2024; year <= until; year++) {
    const offset = year - 2023;
    const predicted = Math.round((start + growth * offset) * mult * (district === 'National' || district === 'National (all districts)' ? 1 : districtFactor * 10));
    ...
    table.push({
      ...
      risk: riskFromCases(district === 'National' || district === 'National (all districts)' ? predicted : predicted * 15),
    });
  }
