...
import { defaultForecastContext, migrateForecastContext } from '../utils/forecastContextUtils';
...
export function AppProvider({ children }) {
  ...
  const [forecastContext, setForecastContext] = useState(() => {
    const saved = localStorage.getItem('fednutri_forecast_context');
    return saved
      ? migrateForecastContext(JSON.parse(saved))
  ...
  const logout = () => {
    ...
    localStorage.removeItem('fednutri_forecast_context');
    setIsAuthenticated(false);
    setForecastContext(defaultForecastContext);
  };

  const applyForecastContext = (updates) => {
    const next = migrateForecastContext({
      ...forecastContext,
      ...updates,
      generated: true,
    });
    setForecastContext(next);
    localStorage.setItem('fednutri_forecast_context', JSON.stringify(next));
  };
